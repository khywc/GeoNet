...
class LGFA(nn.Module):
    ...
    def __init__(self, in_channel, out_channel_list, k_hat=20, bias=False, ):
        ...
        self.self_feature_learning = nn.Conv1d(in_channel, out_channel, kernel_size=1, bias=bias)
        self.neighbor_feature_learning = nn.Conv2d(in_channel, out_channel, kernel_size=1, bias=bias)
        self.k = k_hat
...
if __name__ == '__main__':
    block = LGFA(in_channel=64, out_channel_list=[64], k_hat=32, bias=False)
    input = torch.rand(1, 64, 10000)
    idx = knn(input, k=32)

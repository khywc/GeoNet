...
if __name__ == '__main__':
    ...
    train_list_path = './dataset/KFold_{}_{}_{}/train_upper_list_{}.csv'.format(kfold_nums, tooth_type, ncells, use_fold)
    test_list_path = './dataset/KFold_{}_{}_{}/test_upper_list_{}.csv'.format(kfold_nums, tooth_type, ncells, use_fold)
    # os.makedirs(os.path.dirname(train_list_path), exist_ok=True)

    # labels_dir = "/home/admin111/code/teethseg_code/dataset"
    labels_dir = "/home/root111/wu_code/dataset"

    model_path = './model/ISL+N2PA4+CTAM+CF'
    ...
    # 初始化自适应联合损失函数
    # loss_fn = AdaptiveJointLoss(num_classes=num_classes, use_cuda=True)
    ...
    # 先检查是否有已保存的模型和训练状态
    checkpoint_path = 'model/ISL+N2PA4+CTAM+CF/DBGANet_15classes_32k_100epochs_10000ncells_4batchsize_5kfold5_0.001lr/latest_checkpoint.tar'
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i_batch, batched_sample in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch + 1}/{num_epochs}")):
            ...
            # print(f'inputs: {inputs}', f'outputs:{outputs}')
